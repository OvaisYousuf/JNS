...
/**
 ...
 <p>
 In order to handle connection setup and tear down properly, I chose to use
 ...
 "Inter-networking with TCP/IP" book series. You can find the diagram both
 ...
 <li>The window size is hard-coded to four.
 </ul>
...
public class MPTCPAgent implements CO_Agent
{
    ...
    private Iterator unreachablesIterator;
    
    private final static double probabilityOfSendingOverUnreachables = 0.05;
    
    private final static double TIME_OUT_MSL = 0.12;
    /**
    ...
    protected MPTCPAgent(int port)
    {
        ...
        this.unreachables = new ArrayList<IPAddr>();
        ipTable = new LinkedMultiValueMap<String, IPAddr>();
        
        random = new Random();
        unreachablesIterator = this.unreachables.iterator();
    ...
    public void send(int length, Object data, int unique_id)
    {
        ...
        packet.data = data;
        packet.pktTime = Simulator.getInstance().getTime();
        System.out.println("Adding packet " + packet.sequence + " to queue from " + m_local_ip + "  at " +  Simulator.getInstance().getTime());
    ...
    private void findIP()
    {
        ...
        while(iter.hasNext() && list.size() > 1)
        {
        	...
        	{
        		if(this.unreachables.contains(m_remote_ip))
        		{
        			iter = list.iterator();
        			counter ++;
        			continue;
    ...
    public void indicate(int status, Object indicator)
    {
        ...
        else if(status == Agent.PACKET_AVAILABLE)
        {
            ...
            if(ippacket.NEG_ACK)
            {
            	...
            	if(!(this.unreachables.contains(ippacket.source)))
            	{
            		
            		this.unreachables.add(ippacket.source);
            		
            		//System.out.println("Received NEG_ACK, adding " + ippacket.source + " to blacklist");
            	}
            	
            	MPTCPPacket mptcppacket = new MPTCPPacket();
            	m_remote_ip = ippacket.source;
            	
            	findIP();
            	m_MPTCP.send(m_local_ip, m_remote_ip, ippacket.length, ippacket.data, m_local_port);
            	if( ((MPTCPPacket)ippacket.data).flags != MPTCPPacket.ACK)
            	{
            		System.out.println("Received NEG_ACK for packet " + ((MPTCPPacket)ippacket.data).sequence + " therefore re sending it at" + Simulator.getInstance().getTime() );
            	} else {
            		System.out.println("Received NEG_ACK for ACK " + ((MPTCPPacket)ippacket.data).ack + " therefore re sending it" + Simulator.getInstance().getTime());
           ...
           /* if ( this.unreachables.contains(ippacket.source)  )
            {
            	this.unreachables.remove(ippacket.source);
            	System.out.println("removing " + ippacket.source + " from blacklist");
            }*/
            ...
            switch(m_state)
            {
                ...
                case SYN_RECVD:
                    if(packet.flags == MPTCPPacket.ACK)
                    {
                        ...
                        m_user.indicate(Agent.CONNECTION_ESTABLISHED, this);
                        Simulator.verbose("            MPTCP connection established");
                        System.out.println("  MPTCP connection established");
                    }
                    break;
                ...
                case ESTABLISHED:
                    ...
                    if(packet.flags == MPTCPPacket.ACK &&
                            m_send_array.size() > 0)
                    {
                    	System.out.println("Received ACK for packet " + packet.ack + " at " + m_local_ip + " at " + Simulator.getInstance().getTime());
                    	System.out.println("Inside indicate : Current position is " +  m_window_pos);
                        while( m_send_array.size() > 0 && packet.ack >= ((MPTCPPacket)
                                m_send_array.elementAt(0)).sequence)
                        {
                        	System.out.println(" ACK received is " + packet.ack + " ,needed ack " + ((MPTCPPacket)m_send_array.elementAt(0)).sequence );
                    ...
                    if(packet.data != null)
                    {
                    	System.out.println("\n Packet sequence number " +  packet.sequence + " received at " + Simulator.getInstance().getTime() + "\n");
                    	if(packet.sequence == m_seq_ack+1)
                    	{
                    		sendPacket(MPTCPPacket.ACK, m_seq_data,
                    				packet.sequence);
                    		m_seq_ack++;
    ...
    protected void update()
    {
                ...
                m_window_pos < m_send_array.size())
        {
            ...
            m_window_pos++;
            System.out.println("Incremented m_window_pos from " + ( m_window_pos-1) + " to " + m_window_pos );
            System.out.println("\n sending packet " + packet.sequence + " from " + m_local_ip + "  at " +  Simulator.getInstance().getTime() + "\n");
    
            // ***** MODIFIED PART ****** //
            
            // if probability returns true send packet on an unreachable path (POS_ACK alternative)
            if(probability() && !this.unreachables.isEmpty()){
            	// reset iterator if end of this.unreachables is reached
           		unreachablesIterator = this.unreachables.iterator();
            	
            	// get next unreachable address
            	IPAddr unreachable;
            	unreachable = (IPAddr) unreachablesIterator.next();
            	
            	// send over that address
            	System.out.println("Sending over unreachable " + unreachable + " packet = " + packet.sequence);
            	m_MPTCP.send(m_local_ip, unreachable , packet.length, packet, m_local_port);
            }
        ...
        if(m_send_array.size() > 0 && m_window_pos <= m_window_size && m_window_pos < m_send_array.size())
  		{
    		//System.out.println(" Pos = " + m_window_pos + " Size = " + m_send_array.size());
  			 Iterator<MPTCPPacket> itr = m_send_array.iterator();
  			 int temp_window_pos=0;
  			 while(itr.hasNext() && temp_window_pos<m_window_pos ){
  				MPTCPPacket packet =   (MPTCPPacket) m_send_array.elementAt(temp_window_pos);
  			 	// If timeout for packet has occured, resend the packet //ak
  				if((Simulator.getInstance().getTime() - packet.pktTime) > TIME_OUT_MSL){ //timeout for our packet MSL =2.0
  					findIP();
  					m_seq_data = packet.sequence;
  					packet.pktTime = Simulator.getInstance().getTime(); 
  					m_MPTCP.send(m_local_ip, m_remote_ip, packet.length, packet, m_local_port);
  					System.out.println("Inside timeout :  Pos = " + m_window_pos + " Size = " + m_window_size + "Temp_pos_window = " + temp_window_pos);
  					System.out.println("Timeout occured..resent packet " + packet.sequence + "  from " + m_local_ip + " at " + Simulator.getInstance().getTime());
  					break;
  				}
  				temp_window_pos++;
  				itr.next();
        ...
        /*if(m_send_array.size() > 0)
        {
        	//System.out.println("avoiding stoppage");
        	Simulator.getInstance().schedule(new MPTCPUpdateCommand(
                    Simulator.getInstance().getTime() + 0.1, this));
        }*/
        
    }
    
    
    // returns a yes/no answer for whether to send a copy of a packet over unreachable or not
    private boolean probability(){
    	...
    	// return probability
    	if(random.nextDouble() <= probabilityOfSendingOverUnreachables)
    		return true;
    	else
    		return false;
    }
