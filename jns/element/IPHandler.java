...
public class IPHandler extends Element implements CL_Agent
{
    ...
    public void update()
    {
        ...
        while(m_packets_send.size() > 0 )
        {
            ...
            {
            	if(((MPTCPPacket)curpacket.data).flags == MPTCPPacket.ACK)
             	{
            		System.out.println("IPHandler ACK at " + m_interfaces.get(0).getIPAddr() + " for packet " + ((MPTCPPacket)curpacket.data).ack + 
             	
            			"  destination is " + curpacket.destination + " size = " + targets.size() + " at  " + Simulator.getInstance().getTime());
             	}
            	else
            	{
            		System.out.println("IPHandler Packet at " + m_interfaces.get(0).getIPAddr() + " for packet " + ((MPTCPPacket)curpacket.data).sequence + 
            	...
            	else if(targets.size() > 0 && ((Interface)targets.get(j)) != null && !((Interface)targets.get(j)).canSend(curpacket.destination, curpacket.length))
                    {
            			if(numRounds == 0)
            				numRounds++;
                	 	if(((Interface)targets.get(j)).isLinkDown() && Preferences.mode == 2)
                	 	{
                	 		if(((MPTCPPacket)curpacket.data).flags == MPTCPPacket.ACK) 
                	 		{
                	 			System.out.println("Link is down, send NEG_ACK for ACK " + ((MPTCPPacket)curpacket.data).ack + "  at " + m_interfaces.get(0).getIPAddr());
                	 		} else
                	 		{
                	 			System.out.println("Link is down, send NEG_ACK for packet " + ((MPTCPPacket)curpacket.data).sequence + "  at " + m_interfaces.get(0).getIPAddr());
                	 	...
                	 	// We can't wait forever. Start dropping them. In case neg_ack is allowed, we return the packet to the sender above.
                	 	if(numRounds >= 3 )
                	 	{
                	 		Simulator.verbose(" Dropping packets from " + curpacket.source + " to " + curpacket.destination);
                ...
                for(int j = 0; j < targets.size(); j++)
                {
                    ...
                    else
                    {
                       ...
                       // Check if we have to fragment...
                        ...
                        if( ((Interface)targets.get(j)) != null && ((Interface)targets.get(j)).getMTU() >= curpacket.length)
                        {
                        ...
                        else if (((Interface)targets.get(j)) != null)
                        {
